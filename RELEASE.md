...
[epic template]: https://github.com/conda-sandbox/downstream/issues/new?assignees=&labels=epic&template=epic.yml
[compare]: https://github.com/conda-sandbox/downstream/compare
[new release]: https://github.com/conda-sandbox/downstream/releases/new
...
[Anaconda Recipes]: https://github.com/AnacondaRecipes/downstream-feedstock
[conda-forge]: https://github.com/conda-forge/downstream-feedstock

# Release Process

> [!NOTE]
> Throughout this document are references to the version number as `OMG!A_RELEASE.[$patch_number]`, this should be replaced with the correct version number. Do **not** prefix the version with a lowercase `v`.

## 1. Open the release issue and cut a release branch. (do this ~1 week prior to release)

> [!NOTE]
> The new release branch should adhere to the naming convention of `OMG!A_RELEASE.x` (make sure to put the `.x` at the end!). In the case of patch/hotfix releases, however, do NOT cut a new release branch; instead, use the previously-cut release branch with the appropriate `OMG!A_RELEASE.x` version numbers.
...
Placeholder for `downstream OMG!A_RELEASE.x` release.
...
[milestone]: https://github.com/conda-sandbox/downstream/milestone/<milestone>
[process]: https://github.com/conda-sandbox/downstream/blob/main/RELEASE.md
[releases]: https://github.com/conda-sandbox/downstream/releases
[main]: https://github.com/AnacondaRecipes/downstream-feedstock
[conda-forge]: https://github.com/conda-forge/downstream-feedstock
[ReadTheDocs]: https://readthedocs.com/projects/continuumio-downstream/
...
- [ ] Create release branch (named `OMG!A_RELEASE.x`)
- [ ] Ensure release candidates are being successfully built (see `conda-canary/label/rc-downstream-OMG!A_RELEASE.x`)
...
- [ ] Merge `OMG!A_RELEASE.x` back into `main`
- [ ] Activate the `OMG!A_RELEASE.x` branch on [ReadTheDocs][ReadTheDocs]
- [ ] Feedstocks
...
```markdown
<details open>  <!-- feel free to remove the open attribute once this section is completed -->
<summary><h4>Patch OMG!A_RELEASE.[$patch_number]</h4></summary>
...
Once the release PRs are filed, successful canary builds will be available on `https://anaconda.org/conda-canary/conda/files?channel=rc-downstream-OMG!A_RELEASE.x` for manual testing.
...
2. Clone and `cd` into the repository if you haven't done so already:
    ...
    ```bash
    (rever) $ git clone git@github.com:/downstream.git
    (rever) $ cd conda
    ```
...
2. Fetch the latest changes from the remote and checkout the release branch created a week ago:
    ...
    (rever) $ git checkout OMG!A_RELEASE.x
    ```

2. Create a versioned branch, this is where rever will make its changes:

    ```bash
    (rever) $ git checkout -b changelog-OMG!A_RELEASE.[$patch_number]
    ```
...
2. Run `rever --activities authors <VERSION>`:
    ...
    - Here's a sample run where we undo the commit made by rever in order to commit the changes to `.authors.yml` separately:
        ...
        ```bash
        (rever) $ rever --activities authors --force OMG!A_RELEASE.[$patch_number]
...
6. Now that we have successfully run the activities separately, we wish to run both together. This will ensure that the contributor list, a side-effect of the authors activity, is included in the changelog activity.
    ...
    - After completing this, you will have at most five commits on your release branch:
        ...
        + 3ec7491f2f58494a62f1491987d66f499f8113ad Update .mailmap
        + 432a9e1b41a3dec8f95a7556632f9a93fdf029fd Update news
        + a5c0db938893d2c12cab12a1f7eb3e646ed80373 Update authorship for OMG!A_RELEASE.[$patch_number]
        + 5e95169d0df4bcdc2da9a6ba4a2561d90e49f75d Update CHANGELOG for OMG!A_RELEASE.[$patch_number]
        ```
...
8. Push this versioned branch.
    ...
    ```bash
    (rever) $ git push -u upstream changelog-OMG!A_RELEASE.[$patch_number]
    ```

9. Open the Release PR targing the `OMG!A_RELEASE.x` branch.
...
11. [Create][new release] the release and **SAVE AS A DRAFT** with the following values:
    ...
    | Field | Value |
    |---|---|
    | Choose a tag | `OMG!A_RELEASE.[$patch_number]` |
    | Target | `OMG!A_RELEASE.x` |
...
2. Find the applicable `OMG!A_RELEASE.x` branch and click the "New pull request" button.

3. "Base" should point to `main` while "Compare" should point to `OMG!A_RELEASE.x`.

4. Ensure that all of the commits being pulled in look accurate, then select "Create pull request".

> [!NOTE]
> Make sure NOT to push the "Update Branch" button. If there are [merge conflicts][merge conflicts], create a temporary "connector branch" dedicated to fixing merge conflicts separately from the `OMG!A_RELEASE.x` and `main` branches.
...
## 9. Open PRs to bump [Anaconda Recipes][Anaconda Recipes] and [conda-forge][conda-forge] feedstocks to use `OMG!A_RELEASE.[$patch_number]`.
